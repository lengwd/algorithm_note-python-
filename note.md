...
这一章其实就是上一章深度搜索和广度搜索的利用

需要注意的添加之后要tail += 1, 完成一个搜索之后需要 head += 1

## 五、最短路径问题

四个算法

（1）flody-warshall 通过邻接图存储 之后一次判断每个点加入是否可以缩短距离 复杂度 N3 可以处理负权环路

（2）dijkstra ： 处理一个点到图中最短路径 就是用一个列表储存 原点 到其他点点距离 然后依次找最小值 进行优化  这里还给了 邻接表有点难理解

就是first[a] = b : a代表的是顶点偏编号 b 是边的编号 next[b] 是 b 对应的下一个边的编号 如果是-1即没有下一条边

next[i] = next[first[edge[0]]]

first[edge[0]] = i

(3) bellman_ford 以边为单位进行 松弛 每次都判断所有边

(4)bellman_ford 以边为单位进行松弛 通过队列的形式记录会松驰的边
...
创建堆 ：从heap_size// 2 开始 下滑操作

删除堆 ：拿掉顶端的 把最末尾的放到顶端 下滑操作
